/*
===========================================================================================================================================
	Filename		SMC.usp_XIRR_10Yr
	Author			John Alton
	Date			11/2015
	Description		Calculate the XIRR for 10 year
...
IF EXISTS (
  SELECT * 
   ...
   WHERE SPECIFIC_SCHEMA = N'SMC'
     AND SPECIFIC_NAME = N'usp_XIRR_10Yr' 
)
   DROP PROCEDURE SMC.usp_XIRR_10Yr
GO

CREATE PROCEDURE SMC.usp_XIRR_10Yr
AS
...
WHERE   A.AccountValid10Yr	= 1 
	AND B.AccountNumber		= A.AccountNumber 
	AND B.SecurityID		= A.SecurityID 
	AND B.TransactionDate	> A.MonthEndLag10Yr
...
UPDATE MPC
	SET MPC.IRR10YrReported = x.xirr
FROM SMC.MonthlyPerformanceCore MPC
