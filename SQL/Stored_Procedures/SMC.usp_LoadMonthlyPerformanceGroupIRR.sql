...
-- Description:	Populate MonthlyPerformanceGroupIRR table
-- 
-- EXEC [SMC].[usp_LoadMonthlyPerformanceGroupIRR]
-- 45:03 IRR Only
...
IF EXISTS (
  SELECT * 
   ...
   WHERE SPECIFIC_SCHEMA = N'SMC'
     AND SPECIFIC_NAME = N'usp_LoadMonthlyPerformanceGroupIRR' 
)
   DROP PROCEDURE SMC.usp_LoadMonthlyPerformanceGroupIRR
GO

CREATE PROCEDURE [SMC].[usp_LoadMonthlyPerformanceGroupIRR]
AS
...
BEGIN
	...
	-- Get the groupings
	INSERT INTO @GroupControl
	SELECT * FROM [SMC].[GroupControl]

	-- Declare variables to hold the data which we get after looping each record 
	DECLARE @GroupColumn VARCHAR(50), @GroupDesc VARCHAR(255)

	-- Truncate group core table
	TRUNCATE TABLE [SMC].[MonthlyPerformanceGroupIRR]

	-- Loop through the rows of a table and insert group calculation to group performance table
	WHILE Exists(SELECT * FROM @GroupControl)
	BEGIN
		-- Get the data from table and set to variables
		SELECT TOP 1 @RowIDCurr = RowID, @GroupColumn = GroupColumn, @GroupDesc = GroupDesc FROM @GroupControl ORDER BY RowID
		...
		SET @sql = '
			...
			INSERT INTO [SMC].[MonthlyPerformanceGroupIRR] (GroupName, GroupDesc, MonthEnd, IRR1M, IRR3M, IRR1Yr, IRR3Yr, IRR5Yr, IRR7Yr, IRR10Yr, IRRCY, IRRJY, SIRR, IRR1MReported, IRR3MReported, IRR1YrReported, IRR3YrReported, IRR5YrReported, IRR7YrReported, IRR10YrReported, IRRCYReported, IRRJYReported, SIRRReported, SMC_LoadDate)
		...
		-- Delete the RowID from Table Variable
		DELETE @GroupControl WHERE RowID = @RowIDCurr		
