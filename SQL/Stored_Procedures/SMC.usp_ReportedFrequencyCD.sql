/*
===========================================================================================================================================
	Filename		SMC.usp_ReportedFrequencyCD
	Author			John Alton
	Date			4/3/2015
...
IF EXISTS (
  SELECT * 
   ...
   WHERE SPECIFIC_SCHEMA = N'SMC'
     AND SPECIFIC_NAME = N'usp_ReportedFrequencyCD' 
)
   DROP PROCEDURE SMC.usp_ReportedFrequencyCD
GO

CREATE PROCEDURE SMC.usp_ReportedFrequencyCD
AS
...
,CTE_QTR AS (
	SELECT 
		...
		END	QtrNumber
		,convert(date, DATEADD(day,-1,DATEADD(month,MONTH(MonthEndDate),DATEADD(year,YEAR(MonthEndDate)-1900,0))) ) AS SemiAnnual
		,DATEADD(yy, DATEDIFF(yy,0,MonthEndDate) + 1, -1) AS YearEndDate
	FROM CTE_Months 
)
,CTE_AccountsCD AS (
	SELECT A.MonthlyPerformanceCoreID
    ,A.AccountNumber
	,A.SecurityID 
	...
	INNER JOIN [SMC_DB_ASA].[asa].[Securities] S ON S.[MellonSecurityId] = A.SecurityID
	INNER JOIN [SMC_DB_ASA].[asa].[Companies] C ON C.[CompanyId] = S.[CompanyId]
	INNER JOIN [SMC_DB_ASA].[asa].[Lookups] FL ON FL.[LookupId] = C.[Frequency]
	LEFT JOIN CTE_QTR D ON D.MonthEndDate = A.ReportedDate
	WHERE A.DataSource = 'CD' AND A.RowType = 'R'	
)
...
UPDATE [SMC].[MonthlyPerformanceCore]
	SET [ReportedPct] = 100
WHERE DataSource = 'CD' AND RowType = 'A'
