/*
===========================================================================================================================================
	Filename		SMC.usp_XIRR_YTD
	Author			John Alton
	Date			11/2015
	Description		Calculate the XIRR for all year to date
...
IF EXISTS (
  SELECT * 
   ...
   WHERE SPECIFIC_SCHEMA = N'SMC'
     AND SPECIFIC_NAME = N'usp_XIRR_YTD' 
)
   DROP PROCEDURE SMC.usp_XIRR_YTD
GO

CREATE PROCEDURE SMC.usp_XIRR_YTD
AS
...
SET @SQLCmd = 'INSERT INTO #cf SELECT A.AccountNumber,A.SecurityID,A.MonthStart,A.MonthEnd,A.StartOfYear,TransactionDate,TransactionAmt,A.Group'
...
BEGIN
	select '(' + '''' + 'EAMV' + '''' + ',' + CAST(Amount AS VARCHAR(20)) + ',' + '''' + CAST(dt AS VARCHAR(20)) + '''' + ',NULL)' as [IN]
	from #xirr
