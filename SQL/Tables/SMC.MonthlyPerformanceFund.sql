...
IF EXISTS (
  SELECT *
   ...
   WHERE sys.schemas.name = N'SMC'
	 AND sys.tables.name = N'MonthlyPerformanceFund'
)
  DROP TABLE [SMC].MonthlyPerformanceFund
GO
/****** Object:  Table [SMC].[MonthlyPerformanceFund]    Script Date: 8/19/2015 2:40:25 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE TABLE [SMC].[MonthlyPerformanceFund](
	[MonthlyPerformanceFundID] [bigint] IDENTITY(1,1) NOT NULL,
 ...
 CONSTRAINT [PK_MonthlyPerformanceFund] PRIMARY KEY CLUSTERED 
(
	[MonthlyPerformanceFundID] ASC
...
ALTER TABLE [SMC].[MonthlyPerformanceFund] ADD  CONSTRAINT [DF_MonthlyPerformanceFund_SMC_LoadDate]  DEFAULT (getdate()) FOR [SMC_LoadDate]
GO



--alter table [SMC].[MonthlyPerformanceFund]
